...
def processing_ncode(input_df: pd.DataFrame):
    ...
    tmp_df['_ncode_chr2num'] = tmp_df['_ncode_chr'].map(
        lambda x: _processing(x))
...
for col_name in train_title_df.columns:
    train_title_df = train_title_df.rename(
        columns={col_name: f"title_{col_name}"})
for col_name in train_story_df.columns:
    train_story_df = train_story_df.rename(
        columns={col_name: f"story_{col_name}"})
for col_name in train_keyword_df.columns:
    train_keyword_df = train_keyword_df.rename(
        columns={col_name: f"keyword_{col_name}"})
for col_name in test_title_df.columns:
    test_title_df = test_title_df.rename(
        columns={col_name: f"title_{col_name}"})
for col_name in test_story_df.columns:
    test_story_df = test_story_df.rename(
        columns={col_name: f"story_{col_name}"})
for col_name in test_keyword_df.columns:
    test_keyword_df = test_keyword_df.rename(
        columns={col_name: f"keyword_{col_name}"})
...
for col_name in train_title_univ_df.columns:
    train_title_univ_df = train_title_univ_df.rename(
        columns={col_name: f"title_univ_{col_name}"})
for col_name in test_title_univ_df.columns:
    test_title_univ_df = test_title_univ_df.rename(
        columns={col_name: f"title_univ_{col_name}"})
...
for col_name in train_story_univ_df.columns:
    train_story_univ_df = train_story_univ_df.rename(
        columns={col_name: f"story_univ_{col_name}"})
for col_name in test_story_univ_df.columns:
    test_story_univ_df = test_story_univ_df.rename(
        columns={col_name: f"story_univ_{col_name}"})
...
for col_name in train_keyword_univ_df.columns:
    train_keyword_univ_df = train_keyword_univ_df.rename(
        columns={col_name: f"keyword_univ_{col_name}"})
for col_name in test_keyword_univ_df.columns:
    test_keyword_univ_df = test_keyword_univ_df.rename(
        columns={col_name: f"keyword_univ_{col_name}"})
...
story_type_train.columns = ['story_' +
                            colname for colname in story_type_train.columns]

# test_story
story_type_test = create_type_features(df_test["story"])
story_type_test.columns = ['story_' +
                           colname for colname in story_type_test.columns]

# train_title
title_type_train = create_type_features(df_train["title"])
title_type_train.columns = ['title_' +
                            colname for colname in title_type_train.columns]

# test_title
title_type_test = create_type_features(df_test["title"])
title_type_test.columns = ['title_' +
                           colname for colname in title_type_test.columns]
...
le_df = pd.DataFrame(data={"userid_le": userid_label,
                     "writer_le": writer_label})
...
df_train = pd.concat(
    [
        df_train[["general_firstup"]],  # 目的変数
        df_train_num.drop(columns = ['userid','end','isstop','isr15','isbl','isgl','istensei', 'iszankoku','istenni','pc_or_k']),
        train_title_df, train_story_df, train_keyword_df,  # roberta系
        story_type_train, title_type_train,
        train_label_df,  # category変数
        train_title_univ_df, train_story_univ_df, train_keyword_univ_df,  # univ系
    ],
    axis=1)
df_test = pd.concat(
    [
        df_test_num.drop(columns = ['userid','end','isstop','isr15','isbl','isgl','istensei','iszankoku','istenni','pc_or_k']),
        test_title_df, test_story_df, test_keyword_df,  # roberta系
        story_type_test, title_type_test,
        test_label_df,  # category変数
...
# 学習データの期間を変更してみる
# 不均衡感をなくす
df_train["datetime"] = df_train['general_firstup'].apply(
    lambda x: datetime.datetime.strptime(x, '%Y-%m-%d %H:%M:%S').date())

df_train_2021 = df_train.query(
    "fav_novel_cnt_bin == 0")[df_train["datetime"] > datetime.date(2021, 1, 1)].drop(columns=["datetime", "general_firstup"])

df_train_1 = df_train.query("fav_novel_cnt_bin == 1").reset_index(drop=True)[
    df_train["datetime"] > datetime.date(2020, 1, 1)].drop(columns=["datetime", "general_firstup"])

df_train_234 = df_train.query("fav_novel_cnt_bin != 0 & fav_novel_cnt_bin != 1").reset_index(
    drop=True)[
    df_train["datetime"] > datetime.date(2020, 1, 1)].drop(columns=["datetime", "general_firstup"])

df_train = pd.concat(
    [df_train_2021, df_train_1, df_train_234]).reset_index(drop=True)
